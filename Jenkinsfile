...
def buildStep(ext) {
	...
	try{
		stage("Building ${ext}...") {
			...
			sh "cd ${env.WORKSPACE}/ && make  CC=\"ccache ${ext}-gcc --sysroot=/opt/toolchains/${ext}/\" CPP=\"ccache ${ext}-cpp --sysroot=/opt/toolchains/${ext}/\" CXX=\"ccache ${ext}-g++ --sysroot=/opt/toolchains/${ext}/\" -j8 "
