...
def buildStep(ext) {
	...
	try{
		stage("Building ${ext}...") {
			...
			sh "cd ${env.WORKSPACE}/ && CC=\"ccache ${ext}-gcc --sysroot=/opt/toolchains/${ext}/\" HOST_LIBPNG=\"-lpng16\" HOST_STRIP=\"${ext}-strip\" HOST_CFLAGS=\"-noixemul\" HOST_LDFLAGS=\"-noixemul\" CPP=\"ccache ${ext}-cpp --sysroot=/opt/toolchains/${ext}/\" CXX=\"ccache ${ext}-g++ --sysroot=/opt/toolchains/${ext}/\" make -j8 "
